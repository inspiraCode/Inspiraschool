<?php
	...
	class API extends REST{
		 ...
		 *
		 */
		private function maestro_materias() {
			// Validate token
			$jwt = $this->getJWT();
			if($jwt!=""){
				try{
				...
				." where cat_teacher.user_name = 'meliton.gonzalez' and cat_group.period = 'SEPTIEMBRE-DICIEMBRE'";
				$r = $this->conn->query($query) or die($this->conn->error.__LINE__);
				$rows = array();
				// Obtener los datos de mysql y llenarlos en objeto de php
				while($row = $r->fetch_assoc()){
		 ...
		 *
		 */
		private function alumnos_calificacion() {
			// Validate token
			$jwt = $this->getJWT();
			if($jwt!=""){
				try{
				...
				." where metter_course.id_metter_course = " 261 
